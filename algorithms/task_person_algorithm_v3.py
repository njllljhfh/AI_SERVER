...
def prepare_people_data(filename):
...
def prepare_matching_matrix_data(filename):
    data = read_json_file(filename)
    # 检查数据中是否存在"data"键
    if "data" in data:
...
def prepare_bushu_data(filename):
    data = read_json_file(filename)
    task_info = {}
...
def prepare_scheduling_data(filename):
    data = read_json_file(filename)
    scheduling_info = {}
...
#获取值班人员与机动人员信息
def execute_query_and_show_remaining(scheduling_filename, query_info_filename, people_info):
    scheduling_info = prepare_scheduling_data(scheduling_filename)
    time_info = read_time_info(query_info_filename)
    ...
    for position_name, position_info in matching_matrix_info.items():
        ...
        for person_str, score in position_info.items():
            ...
            if person in people_info:
                ...
                updated_score = max(score-frequency / 10, 0)
                # 保留三位小数
...
def update_data(queried_people,people_info,matching_matrix_filename):
    # 读取人员信息
    matching_matrix_info = prepare_matching_matrix_data(matching_matrix_filename)
...
def check_task_completion_1(people_position, position_remaining_required_people,a):
    total_remaining_required_people = sum(position_remaining_required_people.values())
    sorted_position_remaining_required_people = dict(sorted(position_remaining_required_people.items(), key=lambda x: int(x[0].replace("岗位", ""))))
    if total_remaining_required_people == 0:
        print("任务可以完成")
...
def out_1(people_position, position_remaining_required_people,a):
    total_remaining_required_people = sum(position_remaining_required_people.values())
    sorted_position_remaining_required_people = dict(
        sorted(position_remaining_required_people.items(), key=lambda x: int(x[0].replace("岗位", ""))))
    # 将数据存储到一个字典中
...
def task_assignment_choice(filename):
    data = read_json_file(filename)
...
#选择人员后检验
def check_task_completion_2(people_position,people_position_de,change_people_position):
    # 准备一个消息模板用于显示置换方案
...
#选择人员后的输出
def out_2(people_position, position_remaining_required_people,people_position_de,change_people_position):
    total_remaining_required_people = sum(position_remaining_required_people.values())
    # 对岗位信息按岗位编号进行排序
    sorted_position_remaining_required_people = dict(
        sorted(position_remaining_required_people.items(), key=lambda x: int(x[0].replace("岗位", ""))))
    ...
    # 初始化数据字典
    data = {
        "任务状态": "任务可以完成,新方案是:" if total_remaining_required_people == 0 else f"任务不可以完成，任务缺失{total_remaining_required_people}人",
        "岗位信息": [],
        "置换方案": []
    }
...
total_people = 0
task_info = prepare_bushu_data("/home/phytium/projects/AI_SERVER/test_data/algo3/task_4G_10P_simple.json")
#print(task_info)
scheduling_filename = '/home/phytium/projects/AI_SERVER/test_data/algo3/scheduling_5day_4geng_4gang.json'
query_info_filename = '/home/phytium/projects/AI_SERVER/test_data/algo3/task_time1.json'
people_info_filename = '/home/phytium/projects/AI_SERVER/test_data/algo3/people19.json'
matching_matrix_filename='/home/phytium/projects/AI_SERVER/test_data/algo3/matching_matrix19_4gang.json'
#获取人员信息
people_info = prepare_people_data(people_info_filename)
#获取值班人员和机动人员信息
remaining_people_info, queried_people=execute_query_and_show_remaining(scheduling_filename, query_info_filename, people_info)
#更新匹配矩阵，把值更人员信息清0
update_matching_matrix_zhigeng=update_data(queried_people,people_info,matching_matrix_filename)
#selected_position, required_people=select_position_by_priority(task_info)
#print(selected_position, required_people)
#updated_matching_matrix,position_info=update_matching_matrix_3(update_matching_matrix,selected_position)
#selected_people=people_group_selection(position_info,required_people)
...
# 读取JSON文件并判断code值
choice_data = read_json_file('/home/phytium/projects/AI_SERVER/test_data/algo3/choice_0.json')
if choice_data['code'] == 0:
    ...
    check_task_completion_1(people_position, position_remaining_required_people,len(remaining_people_info))
    out_1(people_position, position_remaining_required_people,len(remaining_people_info))
    #读入系统选择
    people_position_or,people_position_de=task_assignment_choice('/home/phytium/projects/AI_SERVER/test_data/algo3/people_choice.json')
    ...
    #更新匹配矩阵
    update_matching_matrix_choice=update_matching_matrix_2(update_matching_matrix_zhigeng, assignmen_person_ids )
    ...
    while len(change_position_completed) < len(people_position_de):
        ...
        position_info = update_matching_matrix_3(update_matching_matrix_choice, selected_position, change_people_position)
        selected_people, remaining_required_people = people_group_selection(position_info, required_people)
    ...
    out_2(people_position, position_remaining_required_people,people_position_de,change_people_position)
    #print(remaining_people_info)
   #print("已执行out函数，并生成了output_suc.json文件。")
